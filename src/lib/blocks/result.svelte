...
function render(apidata) {
  ...
  films.forEach((item) => {
    ...
    let rating = item.ratingKinopoisk ?? item.rating ?? item.ratingImdb ?? "-";
    let desc = item.description ?? genr ?? linkType ?? "-";

    result = [...result, {id: id, poster: poster, name: name, year: year, rating: rating, desc: desc}]
  })
  $status = "ready"
}
...
const Navigate = (event) => {
  ...
  if (urlvars.page) {
    ...
    newhref = location.search.replace("&page="+urlvars.page, "&page="+event.detail.newPage)
  } else {
    $req.page = 2
...
<div class="result">
  <ul class="result__list">
    ...
    {#if $status == "ready"}
      {#each result as f}
    		<Card id={f.id} poster={f.poster} name={f.name} year={f.year} rating={f.rating} desc={f.desc} />
    	{/each}
    {/if}
  ...
  {#if $status == "ready"}
    <Nav pagesCount={pagesCount} curPage={curPage} on:Nav={Navigate}/>
  {/if}
</div>
{/if}
