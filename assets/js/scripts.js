...
async function search_parse(type, page) {
  ...
  if (type === 'filters') {
    ...
    if (response.ok) {
      ...
      var next = page + 1;
      if (page == 1) {
        $('#search-list').append('<input type="button" value="Страница ' + next + '" onclick="search_parse(`' + type + '`, 2)" class="subbutton" style="display: flex;">');
      } else {
        $('#search-list').append('<div style="display: flex;"><input type="button" value="Страница ' + prew + '" onclick="search_parse(`' + type + '`, ' + prew + ')" class="subbutton"> <input type="button" value="Страница ' + next + '" onclick="search_parse(`' + type + '`, ' + next + ')" class="subbutton"></div>')
      };
    } else {
      alert('error', response.status);
    }
